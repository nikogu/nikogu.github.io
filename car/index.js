  ...
  var perform = function(i) {
    ...
    if (!shifting) {
      if ($scope.currentGear <= $scope.gears.length) {
        ...
        if ($scope.currentSpeed > $scope.maxSpeed) {
          $scope.currentSpeed = $scope.maxSpeed;
        }
...
(function() {
  ...
  // cars
  var cars = [BMW_M2, FOCUS_ST, FOCUS_RS, AUDI_RS3, BENZ_A45, Ferrari_458];
  ...
  function go() {
    ...
    cars.forEach(function(car) {
      ...
      if (car.x >= distance) {
        ...
        car.node.querySelector('span').innerHTML +=
          " " + (new Date().getTime() - beginTime) / 1000 + "s";
